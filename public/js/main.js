...
for (var i = 0; i<35; i++){
  map_flow_water_down();
  //memory leak in pressure function
  map_calculate_pressure();
  // console.log("++++++++++++++++++++++")
  // map_print();
  // console.log("++++++++++++++++++++++")
  map_flow_water_across();
  console.log("======================")
  map_print();
  console.log("======================")
...
function map_add_wall(){
  for (var col = 5; col <6; col++){
      for (var row=0; row <ROWS-1; row++){
          map[col][row]= -1;
      }
    }
    for (var col = 7; col <8; col++){
...
function map_calculate_pressure(){
    ...
    for (var col=0; col < COLS; col++){
        ...
        for (var row = 0; row <ROWS; row ++){
            ...
            // set the pressure to be what you find it at the first/top square
            var pressure = map[col][row];
            if(water_found){
                var is_water = true;
                while (is_water){
...
function map_flow_water_across(){

  //for each row
  for(row = ROWS; row>0; row--){
    var left_pressure = 0;
    var mid_pressure = 0;
    var right_pressure = 0;

    for (var col = 0; col <COLS; col++){
      // check each square, if pressure greater than 1 then send towards lowest pressure
      // don't change pressure if out of range or -1
      if (col ==0){
        left_pressure =-1;
      }
      else{
        left_pressure = map[col-1][row];
      }
      mid_pressure= map[col][row];
      ...
      if (col == COLS-1){
        ...
        right_pressure = map[col+1][row];
      }
      //if the mid pressure is higher than 1


      if (mid_pressure>1){
        var left_pressure_changed = false;
        var right_pressure_changed = false;

        if (right_pressure < mid_pressure && right_pressure > -1){
          right_pressure ++;
          mid_pressure --;
          right_pressure_changed = true
        }
        if (left_pressure < mid_pressure && left_pressure > -1){
          left_pressure ++;
          mid_pressure --;
          left_pressure_changed  = true;
        }




          // adjust the pressure for each column affected in the change
          var pressure_changed = false;

          if (left_pressure_changed){
            map[col-1][row] = left_pressure;
            pressure_changed = true;
          }

          if (right_pressure_changed){
            map[col+1][row] = right_pressure;
            pressure_changed = true;
          }

        if (pressure_changed){
          map[col][row] = mid_pressure;
          // go up each column starting at the row we're on and adjust the pressures until you reach 0

          var l_press = left_pressure;
          var m_press = mid_pressure;
          var r_press = right_pressure;

          for(var new_row =row-1; new_row>-1; new_row --){

            if(l_press>0 && map[col-1][new_row]>-1){
              l_press --;
              map[col-1][new_row] = l_press;

            }
            if(m_press>0 && map[col][new_row]>-1){
              m_press --;
              map[col][new_row] = m_press;

            }
            if(r_press>0 && map[col+1][new_row]>-1){
              r_press --;
              map[col+1][new_row] = r_press;
      ...
      //check each square, hold left, middle and right
      // decide if middle square needs to go left or right
      // if it needs to move then -1 and add one to left or right

    }
  }
}

function map_remove_top_water(){
  // for
}
...
//     var wall_1  = this.matter.add.image(-25, HEIGHT/2, 'wall_image').setStatic(true);
//     wall_1.setCollisionCategory(this.water_collision_cat);
//     var wall_2  = this.matter.add.image(WIDTH-25, HEIGHT/2, 'wall_image').setStatic(true);
//     wall_2.setCollisionCategory(this.water_collision_cat);
//
//     var floor_1  = this.matter.add.image(WIDTH/2, -50, 'floor_image').setStatic(true);
//     floor_1.setCollisionCategory(this.water_collision_cat);
//     var floor_2  = this.matter.add.image(WIDTH/2, HEIGHT+25, 'floor_image').setStatic(true);
//     floor_2.setCollisionCategory(this.water_collision_cat);
//
//     // an event that stops the water rising if it is
// //    this.time.addEvent({ delay:100, callback: new_ball, callbackScope: this, loop: true });
//
//     this.time.addEvent({delay: 200, callback: map_flow_water_down, callbackScope: this, loop: true});
//     this.time.addEvent({delay: 250, callback: map_calculate_pressure, callbackScope: this, loop: true});
